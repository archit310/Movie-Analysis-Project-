...
<?php
$query="SELECT title,release_date,CEIL(avg(rating)) as avg_rating,COUNT(*) as cnt FROM movies m JOIN ratings r ON m.id=r.movie_id JOIN users u ON u.id = r.user_id JOIN occupations o ON o.id = u.occupation_id WHERE name='".$x."' group by title HAVING COUNT(*)>15";
